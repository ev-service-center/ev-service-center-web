...
import { ArrowLeft, Save, Package, DollarSign, Clock, AlertTriangle } from 'lucide-react';
...
const EditDecalServicePage = () => {
    ...
    useEffect(() => {
        if (service) {
            setFormData({
                ...
                standardWorkUnits: service.standardWorkUnits ? service.standardWorkUnits.toString() : '',
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }) => decalService.updateDecalService(id, data),
        onSuccess: () => {
            toast.success('Đã cập nhật dịch vụ decal thành công!');
            queryClient.invalidateQueries(['decalServices']);
            queryClient.invalidateQueries(['decalService', id]);
            navigate(`/decal-services/${id}`);
        },
        onError: (error) => {
            console.error('Update decal service error:', error);
            const errorMessage = error.response?.data?.message || error.message || 'Có lỗi xảy ra khi cập nhật dịch vụ decal';
            toast.error('Lỗi khi cập nhật dịch vụ decal: ' + errorMessage);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
                <Button
                    variant="ghost"
                    onClick={() => navigate(`/decal-services/${id}`)}
                    className="p-2"
                >
                    <ArrowLeft className="w-4 h-4" />
                </Button>
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Chỉnh sửa Dịch vụ Decal</h1>
                    <p className="text-gray-600">Cập nhật thông tin dịch vụ: {service.serviceName}</p>
                </div>
            </div>
...
export default EditDecalServicePage;
